...
| Variable | Effect |
|---|---|
| `SPYRE_INDUCTOR_LOG=1` | *Deprecated*. Use `SPYRE_LOGS="spyre.inductor:INFO"`. Enables Spyre-specific Inductor logging |
| `SPYRE_INDUCTOR_LOG_LEVEL=DEBUG` | *Deprecated*. Set the level in `SPYRE_LOGS` (e.g. `spyre.inductor:DEBUG`). Sets Spyre Inductor log verbosity |
...
| `SPYRE_LOGS="spyre.inductor:DEBUG"` | Preferred logging control. Accepts `spyre.*` namespaces (`spyre`, `spyre.inductor`, `spyre.inductor.codegen`, and so on) |
