...
def _clean_env(overrides: dict) -> dict:
    """Return a copy of the current environment with legacy vars stripped."""
    env = os.environ.copy()
...
def _run_subprocess(script: str, env_overrides: dict) -> subprocess.CompletedProcess:
...
class TestTorchLogsEnablesRuntimeDebug:
    ...
    def test_cpp_logging_config_reflects_torch_logs(self):
        ...
        result = _run_subprocess(script, {"TORCH_LOGS": "spyre.runtime:DEBUG"})
        assert result.returncode == 0, f"Subprocess failed: {result.stderr}"
        if "CPP_ENABLED=SKIP" in result.stdout:
            pytest.skip("_C._logging not available in this build")
...
class TestOutputFormatMatchesSpec:
    ...
    def test_output_format_matches_spec(self):
        ...
        result = _run_subprocess(script, {"TORCH_LOGS": "spyre.runtime:DEBUG"})
        assert result.returncode == 0, f"Subprocess failed: {result.stderr}"
        if "ENABLED=SKIP" in result.stdout:
            pytest.skip("_C._logging not available in this build")
